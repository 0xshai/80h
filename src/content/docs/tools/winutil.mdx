...
<Aside type="tip">
`christitus.com/win` 是官方短链，等价于从 GitHub 拉取最新脚本，每次运行都是最新版本。需开代理访问。
</Aside>

<Aside type="caution">
必须以**管理员身份**运行，否则大部分功能无法生效。右键开始菜单 → Windows Terminal（管理员）。
...
<Aside type="caution">
这里的每一项都有明确适用场景，**不要全勾**。先理解每项的作用再决定是否启用。
</Aside>
...
<Aside type="caution">
完全禁用更新会带来安全风险。有问题更新时临时禁用，问题修复后记得重新开启。
</Aside>
...
<Aside type="tip">
WinUtil 配置可以导出为 JSON 文件，方便在多台设备上重复应用相同设置。
</Aside>
