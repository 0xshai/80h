...
<Aside type="tip">
Bing 的数据会同步给 DuckDuckGo 和 Yahoo，所以提交一次相当于覆盖了好几个搜索引擎。
</Aside>
...
<Aside type="caution">
百度爬虫无法访问 `pages.dev`、`github.io` 等境外托管域名。如果你的网站部署在这些平台上，提交了也没有实际效果。绑定自定义域名（如 `.com`、`.cn`）并确保国内可访问后再操作。
</Aside>
...
2. 输入域名，选择站点类型
3. 验证方式选 HTML 文件，下载后放进 `public/` 部署
