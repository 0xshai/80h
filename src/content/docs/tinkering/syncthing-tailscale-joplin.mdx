...
<Aside type="caution">
默认情况下，Joplin 的"文件系统同步"不会主动传播删除操作：电脑删了笔记 → Joplin 只在本机数据库标记删除 → Syncthing 没有收到删除指令 → 手机端文件还在 → 手机 Joplin 重新读入后笔记仍存在。
</Aside>
...
<Aside type="caution">
Scoop 版是纯命令行，无 GUI，需手动管理启动。MSI 版含系统服务 + 托盘图标 + 开机自启，安装即用。
</Aside>
...
- Play Store / F-Droid 安装 **Tailscale**
- 登录**同一个账号**
